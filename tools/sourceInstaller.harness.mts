import { installResolvedSchemaSource } from "../src/packs/sourceInstaller";
import type { ResolvedGithubSource } from "../src/packs/githubSources";
import type { SchemaSource } from "../src/packs/sources";
import { removeSchemaSourceStorage } from "../src/packs/storage";

const files = new Map<string, string | ArrayBuffer>();
const folders = new Set<string>([".obsidian/notebook/sources"]);
const adapter = {
...
const content: Record<string, string> = {
	"notebook.json": JSON.stringify({ manifestVersion: 1, repository: "owner/repo", packs: [{ id: "test", version: "1.0.0", path: "notebook/test/pack.json" }] }),
	"notebook/test/pack.json": JSON.stringify({ manifestVersion: 1, version: "1.0.0", minimumNotebookVersion: "2.7.0", requires: [], pack: { id: "test", label: "Test", style: {}, assets: { images: { paper: "paper.png" }, stylesheets: ["styles/base.css", "styles/print.css"] } } }),
};
...
const root = ".obsidian/notebook/sources/owner--repo";
...
const unsafeContent: Record<string, string> = {
	"notebook.json": JSON.stringify({ manifestVersion: 1, repository: "owner/repo", packs: [{ id: "unsafe", version: "1.0.0", path: "notebook/unsafe/pack.json" }] }),
	"notebook/unsafe/pack.json": JSON.stringify({ manifestVersion: 1, version: "1.0.0", minimumNotebookVersion: "2.7.0", requires: [], pack: { id: "unsafe", label: "Unsafe", style: {}, assets: { stylesheets: ["../outside.css"] } } }),
};
...
const rootedContent: Record<string, string> = {
	"notebook.json": JSON.stringify({ manifestVersion: 1, repository: "owner/repo", packs: [{ id: "rooted", version: "1.0.0", path: "notebook/rooted/pack.json" }] }),
	"notebook/rooted/pack.json": JSON.stringify({ manifestVersion: 1, version: "1.0.0", minimumNotebookVersion: "2.7.0", requires: [], pack: { id: "rooted", label: "Rooted", style: {}, assets: { root: "media", images: { paper: "paper.png" } } } }),
};

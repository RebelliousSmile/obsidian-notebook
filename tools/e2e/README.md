...
The journey temporarily replaces the Notebook plugin and storage directories in the selected vault. It atomically isolates their original contents outside the vault, restores them on every exit through a shell trap, verifies the restored trees, and preserves the backup if verification fails. It refuses to start while Obsidian is already running.
...
- a disposable or backed-up vault where the `obsidian-notebook` community plugin is enabled.

Run:

```bash
NOTEBOOK_E2E_ALLOW_MUTATION=1 \
NOTEBOOK_E2E_VAULT=/absolute/path/to/test-vault \
NOTEBOOK_E2E_OBSIDIAN=/absolute/path/to/Obsidian.AppImage \
pnpm e2e:request-url
```

Optional variables:

- `NOTEBOOK_E2E_PLUGIN_DIR`: directory containing `main.js`, `manifest.json`, and `styles.css`; defaults to `dist/`.
- `NOTEBOOK_E2E_OUTPUT_DIR`: empty directory for the persistent report and screenshots; defaults to a new directory under `/tmp` and must remain outside the vault.
- `NOTEBOOK_E2E_CDP_PORT`: Electron debugging port; defaults to `9223`.
- `NOTEBOOK_E2E_OBSIDIAN_LOG`: Obsidian log file; defaults to `$HOME/.config/obsidian/obsidian.log`.

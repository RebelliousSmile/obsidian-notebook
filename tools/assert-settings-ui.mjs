...
if (source.includes("Schema sources") || source.includes("renderSchemaSources") || plugin.includes("reloadInstalledSchemaSources")) {
	failures.push("The retired schema source controls are still exposed in Notebook.");
}

if (!source.includes('setHeading("Callouts pour ce pack")')) {
	failures.push("The callout section does not identify its active-pack scope.");
}
...
if (!themeContentsModal.includes("block.mode === packId") || !themeContentsModal.includes("requiredBlocks.has(block.id)")) {
	failures.push("The theme inventory omits code blocks built into the active pack.");
}
